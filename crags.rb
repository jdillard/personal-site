...
def create_crag_page(crags)
  ...
  File.open("crags.html","w") do |f|
    ...
    f << 'precipitation-focused reports of current, past, and future weather conditions for crags located in the United States, sourced'+"\n"
...
def create_crags(crags)
    crags.each do |crag|
      ...
      File.open("_crags/" + slug + "-weather.md","w") do |f|
        ...
        f << "description: Real-time, precipitation-focused reports of current, past, and forecasted weather conditions for " + crag["name"] + ', ' + crag["state"] + "\n"
        f << "js_includes:\n"
        f << "  - weather.js\n"
        f << "---\n\n"
        f << '<section class="measure center lh-copy f5-ns f6 ph2 mv4" style="text-align: justify;">'+"\n"
        f << '<strong>"Is it dry?"</strong>, an oft-repeated, age-old question. Here are real-time,'+"\n"
        f << 'precipitation-focused reports of current, past, and forecasted weather conditions for ' + crag["name"] + ', ' + crag["state"] + ', sourced'+"\n"
        f << 'from <a class="no-underline fancy-link relative light-red" target="_blank" href="https://www.weather.gov/documentation/services-web-api">weather.gov</a>.'+"\n"
        f << "</section>\n\n"
  ...
  metros.each do |metro|
    ...
    File.open("_crags/" + slug + "-weather.md","w") do |f|
      f << "---\n"
        f << "### THIS FILE IS AUTO-GENERATED - DO NOT EDIT ###\n"
        f << "layout: page\n"
        f << "title: " + metro + " Current, Past, and Forecasted Weather Conditions\n"
        f << "title_override: " + metro + " Weather\n"
        f << "description: Real-time, precipitation-focused reports of current, past, and forecasted weather conditions for crags near " + metro + "\n"
        f << "js_includes:\n"
        f << "  - weather.js\n"
        f << "---\n\n"
        f << '<section class="measure center lh-copy f5-ns f6 ph2 mv4" style="text-align: justify;">'+"\n"
        f << '<strong>"Is it dry?"</strong>, an oft-repeated, age-old question. Here are real-time,'+"\n"
        f << 'precipitation-focused reports of current, past, and forecasted weather conditions for crags near ' + metro + ', sourced'+"\n"
        f << 'from <a class="no-underline fancy-link relative light-red" target="_blank" href="https://www.weather.gov/documentation/services-web-api">weather.gov</a>.'+"\n"
        f << "</section>\n\n"
        f << '<p id="settings-toggle" class="mw5 b center tc hover-light-red black-70 pointer">Show Settings</p>'+"\n"
        f << '<section id="settings" class="overflow-hidden" style="transition: height 200ms;height: 0px;">'+"\n"
