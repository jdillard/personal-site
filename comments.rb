...
Dir.glob("source/_posts/*.md") do |blog_post|
  ...
  if front_matter["published"] === true && front_matter["comments"]
    File.open("source/_includes/comments/" + front_matter["comments"].to_s  + ".html","w") do |f|
      if client.issue(repo, front_matter["comments"]).comments > 0
        # Loop over comments and write out comment file
        client.issue_comments(repo, front_matter["comments"]).each do |comment|
